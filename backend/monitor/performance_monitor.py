...
class PerformanceMonitor:
    ...
    def __init__(
        ...
        self._resolved_alerts: Deque[Alert] = deque(maxlen=100)
        self._breach_streaks: Dict[str, int] = defaultdict(int)
        self._recovery_streaks: Dict[str, int] = defaultdict(int)
    ...
    def _check_threshold(self, metric: str, value: float, label: str) -> None:
        ...
        if triggered:
            self._breach_streaks[alert_key] += 1
            self._recovery_streaks[alert_key] = 0
            ...
            if alert is not None:
                ...
                alert.occurrences += 1
                return
            if self._breach_streaks[alert_key] < 3:
                return
            ...
            alert = Alert(
                ...
                value=value,
                threshold=threshold,
                occurrences=self._breach_streaks[alert_key],
            )
        ...
        self._breach_streaks[alert_key] = 0
        if alert is None:
            return

        recovery_threshold = threshold * 0.9 if operator == "greater_than" else min(1.0, threshold + 0.03)
        recovered = (operator == "greater_than" and value <= recovery_threshold) or \
                    (operator == "less_than" and value >= recovery_threshold)
        if not recovered:
            self._recovery_streaks[alert_key] = 0
            return

        self._recovery_streaks[alert_key] += 1
        if self._recovery_streaks[alert_key] < 3:
            return
        ...
        self._recovery_streaks[alert_key] = 0
        logger.info(f"[RESOLVED] {alert.metric} 已恢复，当前值 {value:.3f}")
        if self._webhook:
            asyncio.create_task(self._send_webhook(alert))
    ...
    def summary(self) -> Dict[str, Any]:
        ...
        return {
            "agent_stats":   self._orchestrator.get_stats(),
            "tool_stats":    self._tool_manager.get_stats(),
            "active_alerts": [asdict(a) for a in list(self._active_alerts.values())[-10:]],
            "recent_resolved_alerts": [asdict(a) for a in list(self._resolved_alerts)[-10:]],
            "suggestions":   [
