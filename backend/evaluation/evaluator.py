...
from core.llm_utils import STRUCTURED_OUTPUT_MAX_TOKENS, extract_text_content
...
请按当前系统在本轮实际可完成的范围评价完整性。对于需要订单后台、支付后台等权限的请求，
如果响应已经收集必要字段、说明可执行的处理分支和验证方式，并明确能力边界与下一步，
应视为完整解决了本轮可完成范围；不要因为响应没有虚构已执行不存在的后台操作而扣分。
    ...
    async def judge(
        ...
        try:
            resp = await self._client.messages.create(
                model=self._model, max_tokens=STRUCTURED_OUTPUT_MAX_TOKENS, temperature=0.0,
