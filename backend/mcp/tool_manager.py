...
返回 JSON 数组，例如: ["子查询1", "子查询2", "子查询3"]"""
    ...
    async def search_with_rewrite(
        ...
        # 3. 合并去重：优先使用知识库稳定 ID；相同结果保留最高召回分。
    ...
    def _result_identity(item: Any) -> str:
        ...
        if isinstance(item, dict):
            ...
            content = " ".join(str(item.get("content") or "").casefold().split())
            chunk = str(item.get("chunk") if item.get("chunk") is not None else "")
            payload = f"{title}\n{chunk}\n{content}"
        else:
            payload = " ".join(str(item).casefold().split())
...
只返回 JSON 数组，不要其他文字。"""
        ...
        try:
            ...
            valid_order = []
            for index in order:
                if isinstance(index, int) and 0 <= index < len(items) and index not in seen_indices:
                    seen_indices.add(index)
                    valid_order.append(index)
            valid_order.extend(index for index in range(len(items)) if index not in seen_indices)
            reranked = [items[index] for index in valid_order]
