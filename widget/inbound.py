...
from widget.conversation_accounts import is_official_or_system_account

_SYSTEM_SENDERS = {"", "filehelper_system"}
...
class InboundFilter:
    ...
    def accept(self, msg: InboundMsg, self_wxid: str) -> bool:
        ...
        if is_official_or_system_account(sender) or \
            ...
            return False
        delivery_replay = bool(msg.get("delivery_replay"))
        if msg["msg_id"] in self._seen and not delivery_replay:  # 去重
            return False
        if not delivery_replay:
