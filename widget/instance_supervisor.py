...
class InstanceSupervisor:
    ...
    def set_status(self, channel_key: str, status: str, error: str = "") -> None:
        """由无独立渠道进程的适配器回报健康状态。"""
        with self._lock:
            if channel_key in self._insts:
                self._insts[channel_key].status = status
                self._insts[channel_key].last_error = error
    ...
    def _go_offline(self, inst: InstanceState) -> None:
        """标 offline 并回调 on_offline（停掉那条可能已经在读别人抖音私信的轮询）。"""
        inst.status = "offline"
            ...
            if now >= self._next_retry.get(inst.channel_key, 0.0):
                ...
                if fn is not None:
                    ...
                    try:
                        ...
                        # 渠道服务启动。这种情况必须
                        # 保持固定短周期重试 —— 指数退避会让「T 时刻开抖音私信」的客户等最多 60s
