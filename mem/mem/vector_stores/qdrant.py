# 中文导读：这是当前本地配置实际使用的向量存储适配器。同一个 Qdrant point
# 同时保存稠密语义向量、可选的 BM25 稀疏向量和原始 payload；Memory 层再把
# 语义、关键词与实体信号融合，而不是依赖 Qdrant 自己给出最终排名。
import logging
import re
...
class Qdrant(VectorStoreBase):
    ...
    def _get_bm25_encoder(self):
        # 延迟加载 fastembed，启动时不需要 BM25 的调用不会承担模型加载成本。
    ...
    def create_col(self, vector_size: int, on_disk: bool, distance: Distance = Distance.COSINE):
        ...
        """
        # 旧 collection 可能只有稠密向量槽位；保持可读并降级，而不是自动迁移
        # 已有数据。要获得完整 hybrid 基线，应换一个全新的 collection 名称。
    ...
    def insert(self, vectors: list, payloads: list = None, ids: list = None):
        ...
        # 先从 payload 的 text_lemmatized 生成稀疏向量。批处理减少 tokenizer 和
        # 模型调度开销；计数不一致或批失败时逐条编码，防止整批关键词索引丢失。
        ...
        for idx, vector in enumerate(vectors):
            ...
            # 空字符串 key 是默认稠密向量；"bm25" 是命名稀疏向量槽位。
    ...
    def search(self, query: str, vectors: list, top_k: int = 5, filters: dict = None) -> list:
        ...
        Returns:
            list: Search results.
        """
        # query 文本仅用于接口一致性；稠密检索实际使用上层已计算好的 vectors。
    ...
    def keyword_search(self, query, top_k=5, filters=None):
        ...
        """
        # 返回 None 表示“当前 collection 没有此能力”，上层会保留语义结果继续运行。
        if not self._has_bm25_slot:
            return None
    ...
    def update(self, vector_id: int, vector: list = None, payload: dict = None):
        ...
        if vector is not None and payload is not None:
            # 文本更新时完整重写稠密向量、BM25 稀疏向量和 payload，三者保持一致。
            named_vectors = {"": vector}
            ...
            # 只改 payload 时不会刷新 BM25 向量；主流程的文本 UPDATE 会传 vector 和
            # payload 走上面的完整更新，因此这里主要服务 linked_memory_ids 等元数据。
    ...
    def close(self):
        """Release local SQLite/Qdrant handles so Windows can reopen the data path."""
