# 中文导读：这是 Memory 与所有向量数据库之间的稳定协议。核心约束是 search()
# 返回“越大越相似”的分数；使用距离的 provider 必须先转换，否则融合排序会反向。
...
class VectorStoreBase(ABC):
    ...
    def keyword_search(self, query: str, top_k: int = 5, filters: dict = None):
        ...
        """
        # None 表示能力不可用，Memory 会自动退化为纯语义检索，不把它当成异常。
        return None
    ...
    def search_batch(self, queries: list, vectors_list: list, top_k: int = 1, filters: dict = None):
        ...
        """
        # 顺序 fallback 定义了正确性基线；Qdrant 等实现可覆写为真正的批查询。
