# 中文导读：SQLite 在本项目中承担两项辅助职责：记录每条记忆的变更审计轨迹，
# 以及保存原始最近消息供下一轮抽取构造 last_k_messages。真正可搜索的记忆正文、
# 向量和过滤 metadata 存在 VectorStore（当前为 Qdrant）中。
import logging
import sqlite3
import threading
import uuid
...
class SQLiteManager:
    ...
    def add_history(
        ...
        *,
        ...
        # sqlite connection 允许跨线程使用，但事务仍由进程内锁串行化。
        with self._lock:
            try:
                self.connection.execute("BEGIN")
                self.connection.execute(
    ...
    def save_messages(self, messages: List[Dict[str, Any]], session_scope: str) -> None:
        # session_scope 由 user_id/agent_id/run_id 规范化生成，防止不同主体的最近
        # 对话互相污染抽取上下文。
        if not messages:
            return
        with self._lock:
