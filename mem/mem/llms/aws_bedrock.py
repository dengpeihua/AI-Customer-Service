...
from mem.memory.utils import extract_json
...
class AWSBedrockLLM(LLMBase):
    ...
    def _build_inference_config(self) -> Dict[str, Any]:
        ...
        Anthropic reasoning models reject requests that include both ``temperature``
        and ``topP`` simultaneously, so ``topP`` is omitted for that provider even
        ...
        if top_p is not None:
            if self.provider == "anthropic":
                # Anthropic reasoning models raise a ValidationException when
                # temperature and topP are both present in inferenceConfig.
