# 中文导读：LLM provider 的共同边界。事实抽取只调用 generate_response；
# 参数兼容逻辑集中在基类，避免每个 provider 重复判断推理模型限制。
...
class LLMBase(ABC):
    ...
    def _get_supported_params(self, **kwargs) -> Dict:
        ...
        # 推理模型通常拒绝 temperature/top_p 等传统采样参数，因此采用白名单；
        # 普通模型则走 _get_common_params 保留完整生成控制项。
