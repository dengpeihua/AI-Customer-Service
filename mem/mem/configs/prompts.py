...
# ---------------------------------------------------------------------------
# 中文导读：V3 Prompt 分为两层。ADDITIVE_EXTRACTION_PROMPT 是稳定的 system
# 规则和示例；下面的 builder 每次把旧记忆、最近消息、新消息和日期填入 user
# prompt。当前实现只要求模型返回新增事实及可选关联，不要求它执行论文中的
# UPDATE/DELETE/NOOP 决策；理解实验差异时务必区分这两种算法。
# ---------------------------------------------------------------------------
...
def generate_additive_extraction_prompt(
    ...
    *,
    ...
    """
    # observation_date 描述“事件被观察的时间”，current_date 描述真正运行时间；
    # 两者分开可以让模型把 recently/last week 等相对时间锚定到历史会话日期。
    current_date, observation_date = _resolve_dates(current_date, timestamp)

    # 即使某段为空也保留标题，使 system prompt 示例与实际输入拥有稳定结构。
    sections = []
