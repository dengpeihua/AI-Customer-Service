...
# 中文导读：这里实现的是轻量“实体辅助索引”，并非论文 Mem0g 的关系三元组
# 抽取器。它组合 spaCy NER、专有名词、技术标识符、引号短语和名词短语等候选，
# 再按优先级、置信度和跨度去重。spaCy 不可用时安全返回空列表，混合检索退化为
# semantic + BM25，而不会阻断记忆写入或搜索。
...
def _resolve_candidates(candidates: list[_EntityCandidate]) -> list[tuple[str, str]]:
    # 第一轮按规范化文本去重，保留优先级更高/置信度更高的来源；第二轮解决
    # token span 重叠，防止同一个名字同时以 NER、PROPER、TOPIC 多次出现。
...
def _extract_entities_from_doc(doc) -> list[tuple[str, str]]:
    ...
    """
    # 各 extractor 只“提名”候选，最终接受哪些实体统一交给 _resolve_candidates。
    tokens = list(doc)
    candidates: list[_EntityCandidate] = []
...
def extract_entities(text: str) -> list[tuple[str, str]]:
    ...
    from mem.utils.spacy_models import get_nlp_full
