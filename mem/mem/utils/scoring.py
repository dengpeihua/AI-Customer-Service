...
# 中文导读：融合只重排“语义搜索已经召回”的候选。semantic 是基础相关性，
# BM25 补足专名/关键词，entity boost 补足同实体关联；最终除以本轮可用信号的
# 理论最大值，使某个可选能力缺失时的分数尺度仍大致可比较。
...
def get_bm25_params(query: str, *, lemmatized: Optional[str] = None) -> tuple:
    ...
    # 查询越长，原始 BM25 分通常越大，所以 sigmoid 中点随词数右移。
    if num_terms <= 3:
        return 5.0, 0.7
    elif num_terms <= 6:
        return 7.0, 0.6
...
def score_and_rank(
    ...
    """
    # 分母只计入本轮真正存在的信号；否则 BM25 不可用时所有结果都会被无故压低。
