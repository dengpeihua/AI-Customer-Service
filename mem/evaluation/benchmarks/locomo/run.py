...
# 中文导读：该文件把论文中的实验过程落成可恢复的工程流水线：
# 数据集会话 -> 分块写入 Mem0 -> 每个问题检索 -> Answerer 生成答案 -> Judge 评分
# -> 逐题 checkpoint -> 分类/截断聚合。Mem0 负责“记什么、找什么”，Answerer 和
# Judge 是评测侧模型，三者不要混为同一个 LLM 调用。
...
from benchmarks.common.paths import DATASETS_DIR, RESULTS_DIR
...
DEFAULT_DATASET_DIR = str(DATASETS_DIR / "locomo")
...
def session_to_chunks(turns: list[dict], speaker_a: str, speaker_b: str) -> list[list[dict]]:
    ...
    # LOCOMO 是两人对话数据。这里借用 user/assistant role 传输，但把真实 speaker
    # 名写回 content，避免 Memory 把第二位参与者误解为系统助手本身。
    messages = []
    for turn in turns:
    ...
    # CHUNK_SIZE=1 表示逐 turn 增量导入，对应论文“持续对话中新消息到来即处理”。
    chunks = []
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        "--answerer-model",
        default=os.getenv("DEEPSEEK_MODEL") or "gpt-5",
    ...
    parser.add_argument(
        ...
        or ("deepseek" if os.getenv("DEEPSEEK_API_KEY") else "openai"),
        help="LLM provider (openai, deepseek, anthropic, azure)",
    )
    ...
    parser.add_argument("--output-dir", default=str(RESULTS_DIR / "locomo"), help="Output directory")
    ...
    parser.add_argument("--backend", default="oss", choices=["oss"], help="Local Mem0 OSS backend")
    parser.add_argument("--mem0-host", default=None,
                        help="Local Mem0 server URL (default: http://localhost:8888)")
...
async def async_main() -> None:
    ...
    async def process_conversation(conv_idx: int):
        ...
        # semaphore 限制“同时处理多少段完整会话”；单会话内部仍按时间顺序写入，
        # 否则最近消息上下文和事件先后关系会被破坏。
        async with conv_semaphore:
            ...
            # 第一阶段：把选中会话完整写入记忆库。max_questions 只限制后面的题目，
            # 不会减少这一步的对话/模型调用量。
            ...
            # 第二阶段：逐题执行 search -> answer -> judge，并立刻写独立结果文件，
            # 中断后 --resume 可跳过已有 question_id。
            questions = entry.get("qa", entry.get("qa_pairs", []))
            conv_questions = [
                (qi, qa) for qi, qa in enumerate(questions)
                if qa.get("category") in categories
    ...
    async with mem0:
        await mem0.ensure_available()
        with shutdown:
            tasks = [process_conversation(idx) for idx in conv_indices]
            await asyncio.gather(*tasks)

    # 第三阶段：predict-only 不调用 Judge；完整运行才聚合 cutoff 与分类指标。
