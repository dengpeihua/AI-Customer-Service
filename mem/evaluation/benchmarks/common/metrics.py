...
# 中文导读：评测结果先按“每问题”落盘，再由这里聚合。accuracy 表示分数超过
# pass_threshold 的比例，avg_score 保留连续 judge 分；两者回答不同问题，不能
# 只看其中一个。cutoff 维度用于比较 top-k 检索上下文多少对最终回答的影响。
...
def compute_group_metrics(
    ...
    """
    # 先收集原始 [0,1] 分数，最后统一乘 100，避免分组与总体使用不同单位。
...
def compute_overall_metrics(
    ...
    # 总体主指标采用最大 cutoff；每个 cutoff 的完整对比仍保存在 by_cutoff。
