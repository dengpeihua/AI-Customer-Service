...
# 中文导读：这是评测侧 Answerer/Judge 的模型客户端，不是 Mem0 写入时使用的
# extraction LLM。三种角色可以碰巧配置成同一模型，但调用目的、Prompt 和指标
# 含义完全不同；分析成本与延迟时应分别统计。
...
class LLMClient:
    ...
    Supports OpenAI, DeepSeek, Anthropic, and Azure OpenAI providers. For other
    OpenAI-compatible APIs (e.g., vLLM, Parasail), set provider="openai" with a
    custom base_url.

    Args:
        model: Model identifier (e.g., "gpt-4o", "claude-sonnet-4-20250514").
        provider: One of "openai", "deepseek", "anthropic", "azure".
    ...
    async def generate(
        ...
        """
        # provider 分支只处理 API 差异；上层 benchmark 始终收到纯文本。
        if self.provider == "anthropic":
